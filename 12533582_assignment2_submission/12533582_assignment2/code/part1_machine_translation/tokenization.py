...
class PrepareData:
    ...
    def wordToID(self, en, cn, en_dict, cn_dict):
        """ 
        Convert word to id with the dictionary generated from training english and chinese data
        ...
        """
        out_en_ids = [
            np.array([en_dict.get(word, self.unk_id) for word in sent], dtype=np.int64)
            for sent in en
        ]
        out_cn_ids = [
            np.array([cn_dict.get(word, self.unk_id) for word in sent], dtype=np.int64)
            for sent in cn
        ]

        sorted_pairs = sorted(zip(out_en_ids, out_cn_ids), key=lambda pair: len(pair[0]))
