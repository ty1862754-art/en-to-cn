...
class LearnablePositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        if x.shape[1] > self.seq_len:
            raise ValueError(f"Input sequence length {x.shape[1]} exceeds maximum positional length {self.seq_len}.")
...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: InputEmbeddings, tgt_embed: InputEmbeddings, src_pos: nn.Module, tgt_pos: nn.Module, projection_layer: ProjectionLayer) -> None:
        super().__init__()
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 2048, pe_type: str = "sinusoidal") -> Transformer:
    ...
    if pe_type == "sinusoidal":
        ...
        raise ValueError(f"Unsupported pe_type: {pe_type}. Use 'sinusoidal' or 'learnable'.")
