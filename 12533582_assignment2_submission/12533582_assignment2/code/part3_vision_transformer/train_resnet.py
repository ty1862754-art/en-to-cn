...
def default_output_dir():
    return Path(__file__).resolve().parents[2] / "outputs" / "part3_vision_transformer" / "resnet50_cifar10"
...
def build_transforms(args):
    if args.imagenet_weights or args.use_torch_hub:
    ...
    train_ops = []
    if resize != 32:
        train_ops.append(transforms.Resize((resize, resize)))
    ...
    eval_ops = []
    if resize != 32:
        eval_ops.append(transforms.Resize((resize, resize)))
    eval_ops.extend([transforms.ToTensor(), transforms.Normalize(mean, std)])
...
def build_resnet50(args):
    if args.use_torch_hub:
        model = torch.hub.load("pytorch/vision:v0.10.0", "resnet50", pretrained=True)
    else:
        weights = models.ResNet50_Weights.DEFAULT if args.imagenet_weights else None
        model = models.resnet50(weights=weights)
    if not args.imagenet_weights and not args.use_torch_hub and not args.standard_stem:
...
def main():
    parser = argparse.ArgumentParser(description="Train ResNet50 on CIFAR-10 for comparison with ViT.")
    parser.add_argument("--data-dir", type=Path, default=default_data_dir())
    parser.add_argument("--output-dir", type=Path, default=default_output_dir())
    parser.add_argument("--download", action="store_true")
    parser.add_argument("--imagenet-weights", action="store_true", default=True)
    parser.add_argument("--no-imagenet-weights", action="store_false", dest="imagenet_weights")
    parser.add_argument("--use-torch-hub", action="store_true")
    parser.add_argument("--freeze-backbone", action="store_true", default=True)
    parser.add_argument("--fine-tune-all", action="store_false", dest="freeze_backbone")
    parser.add_argument("--standard-stem", action="store_true")
    ...
    model = build_resnet50(args).to(device)
    ...
    config.update(
        {
            ...
            "pretrained": bool(args.imagenet_weights or args.use_torch_hub),
            "pretrained_source": "torch_hub_pytorch_vision_v0.10.0" if args.use_torch_hub else ("torchvision_default" if args.imagenet_weights else "none"),
        }
    )
